...
def dist_func(args):
    ...
    for k in range(n):
        dist[:,k] = np.sqrt((x1-x2[k])**2 + (y1-y2[k])**2)
#        sp_dist[:,k] = np.arccos(np.clip(np.sin(x1rd)*np.sin(lan2rd[k]) + np.cos(lan1rd)*np.cos(lan2rd[k])*np.cos(lon1rd-lon2rd[k]),-1,1))
    return(dist)
    
def dist(x1,y1,x2,y2,workers,sph_func=dist_func):
    if len(x1)>workers*1000:
        length = workers*1000
    elif 1000<=len(x1)<=workers*1000:
        length = 1000
    else:
        coeff_mat = sph_func([x1,y1,x2,y2])
        return(coeff_mat)
    scale = int(np.floor(length/workers))
    
    size = int(np.floor(len(x1)/length))
    indis = np.arange(0,len(x1),scale)
    indis = np.append(indis,len(x1))
    
    for i in range(size+1):
        if i<=(size-1):
            args = [None]*workers
            for k in range(workers):
                args[k] = [x1[indis[workers*i+k]:indis[workers*i+1+k]],y1[indis[workers*i+k]:indis[workers*i+1+k]],x2,y2]
            ...
            for k in range(workers-1):
                args[k] =  [x1[indis_new[workers*new_i+k]:indis_new[workers*new_i+1+k]],y1[indis_new[workers*new_i+k]:indis_new[workers*new_i+1+k]],x2,y2]
        pool = Pool()
...
#     distance=args[0];shape=args[1];designed_coeff=args[2];functions=args[3]
#     if len(np.shape(designed_coeff))>=1:
#         coeff_matrix = np.empty([np.shape(distance)[0],np.shape(distance)[1]*3],dtype=float)
#         for i in range(np.shape(distance)[0]):
#             index = 0
#             for k in range(0,np.shape(distance)[1]*3,3):
#                 coeff_matrix[i,k]   = functions(distance[i,index],shape[index])*designed_coeff[i,0]
#                 coeff_matrix[i,k+1] = functions(distance[i,index],shape[index])*designed_coeff[i,1]
#                 coeff_matrix[i,k+2] = functions(distance[i,index],shape[index])*designed_coeff[i,2]
#                 index += 1
#         return(coeff_matrix)
#     else:
#         coeff_matrix = np.empty([np.shape(distance)[0],np.shape(distance)[1]],dtype=float)
#         for i in range(np.shape(distance)[0]):
#             for k in range(np.shape(distance)[1]):
#                 coeff_matrix[i,k]   = functions(distance[i,k],shape[k])*designed_coeff[i]
#         return coeff_matrix
...
def multiprocessing_model_coeff_pointmass(distance,shape,designed_coeff,workers,functions,functions_1):
    
    if len(distance)>workers*1000:
        length = workers*1000
    elif 1000<=len(distance)<=workers*1000:
        length = 1000
    else:
        coeff_mat = functions_1([distance,shape,designed_coeff,functions])
        return(coeff_mat)
    
    scale = int(np.floor(length/workers))
    
    size = int(np.floor(len(distance)/length))
    indis = np.arange(0,len(distance),scale)
    indis = np.append(indis,len(distance))
    
    for i in range(size+1):
        if i<=(size-1):
            args = [None]*workers
            for k in range(workers):
                args[k] = [distance[indis[workers*i+k]:indis[workers*i+1+k]],shape,designed_coeff[indis[workers*i+k]:indis[workers*i+1+k]],functions]
            ...
            new_i = 0
            for k in range(workers-1):
                args[k] = [distance[indis_new[workers*new_i+k]:indis_new[workers*new_i+1+k]],shape,designed_coeff[indis_new[workers*new_i+k]:indis_new[workers*new_i+1+k]],functions]
...
def MCVCE(coeff_mat,grav):
    ...
    while True:
        ...
        for i in range(data_number):
            Qd += (1/(sigma[i][-1]**2))*(coeff_mat[i].T @ coeff_mat[i])
            Qn += (1/sigma[i][-1]**2)*np.dot(coeff_mat[i].T,grav[i])
        Qd += (1/(sigma[-1][-1]**2))*Pu
